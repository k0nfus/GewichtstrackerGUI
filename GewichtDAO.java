...
public class GewichtDAO {
	...
	public void createTableIfNotExists() {
		try (Connection conn = DriverManager.getConnection(DATABASE_URL); Statement stmt = conn.createStatement()) {
			String sql = "CREATE TABLE IF NOT EXISTS gewicht (" + "id INTEGER PRIMARY KEY AUTOINCREMENT,"
					+ "datum TEXT NOT NULL," + "gewicht REAL NOT NULL)";
			stmt.execute(sql);
	...
	public void insert(GewichtEntry entry) {
		try (Connection conn = DriverManager.getConnection(DATABASE_URL);
				PreparedStatement pstmt = conn.prepareStatement("INSERT INTO gewicht (datum, gewicht) VALUES (?, ?)")) {
	...
	public List<GewichtEntry> getAllEntries() {
		...
		try (Connection conn = DriverManager.getConnection(DATABASE_URL);
				...
				ResultSet rs = stmt.executeQuery("SELECT * FROM gewicht ORDER BY id DESC")) {
			while (rs.next()) {
				int id = rs.getInt("id");
	...
	public void update(GewichtEntry entry) {
		try (Connection conn = DriverManager.getConnection(DATABASE_URL);
				PreparedStatement pstmt = conn
						.prepareStatement("UPDATE gewicht SET datum = ?, gewicht = ? WHERE id = ?")) {
			pstmt.setString(1, entry.getDatum());
			pstmt.setDouble(2, entry.getGewicht());
	...
	public void delete(int id) {
		try (Connection conn = DriverManager.getConnection(DATABASE_URL);
				PreparedStatement pstmt = conn.prepareStatement("DELETE FROM gewicht WHERE id = ?")) {
			pstmt.setInt(1, id);
			pstmt.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
